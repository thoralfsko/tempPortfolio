...
object VerticalBoxBlur extends VerticalBoxBlurInterface:
  ...
  def parBlur(src: Img, dst: Img, numTasks: Int, radius: Int): Unit = 
    val incDenom = Math.min(numTasks, src.width)//this should handle the case where width < numtask
    val increment = src.width / incDenom
    val range = 0 to src.width by increment
    val chunks = range.zip(range.tail)
